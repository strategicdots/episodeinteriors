...
class DatabaseObject {
    ...
    public static function findAllUnderParent($foreignKeyInParent, $uniqueKeyInChild) {
        global $database;
        
        $sql  = "SELECT * FROM ";
        $sql .= static::$table_name;
        $sql .= " WHERE ";
        $sql .= $database->escapeValue($uniqueKeyInChild) . "=";
        $sql .= $database->escapeValue($foreignKeyInParent);
